...
> **数据一致性:** 数据一致性问题分为好多种, 比如上面用户A, B同时点赞, 导致点赞数丢失的问题, 还有个常见的例子比如多个人给一个账户转 100 块钱,  A事务读取到此时账户余额为 100, B事务 也读取到账户余额为 100, 所以 A: 余额= 100 + 100 = 200, B事务 也是这样, 导致数据一致性问题, **这种数据一致性问题可以通过 x 锁来解决,** 下面我们会讲如何通过 x 锁解决这种问题, 
>
> 当然数据一致性问题还有其他类型, 比如假设你在银行 A 账户有 1000 元，你想转账 200 元到银行 B 账户, 正常情况下 从 A 账户扣除 200 元（余额变成 800）, 向 B 账户增加 200 元（余额变成 1200）, 假设在步骤 1 之后（A 账户变成 800），系统崩溃或网络异常，导致步骤 2 没有执行, 这就导致 A 账户已经减少了 200 元（800），但 B 账户仍然是 1000, 这也是数据一致性问题, **这种数据一致性问题我们可以利用事务的原子性来解决**, 
>
...
对于我们上面的情况, 其实并不需要手动加x锁, 只需要使用事务保证操作的原子性就好了, 因为UPDATE 会自动获得 x 锁, 不必担心并发问题, 如下:
...
至于防止用户重复点赞, 我们可以在 `(post_id, user_id)` 上建立索引, 若相同数据插入, `INSERT INTO post_likes (post_id, user_id) VALUES (1, 1);` 必然失败, 下面的操作也不会发生, 事务也会回滚, 
...
而且我们可以在执行插入, 更新点赞数之前判断点赞数, 具体实现如下:
