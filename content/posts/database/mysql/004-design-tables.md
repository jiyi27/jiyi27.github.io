...
至于防止用户重复点赞, 我们可以在 `(post_id, user_id)` 上建立索引, 若相同数据插入, `INSERT INTO post_likes (post_id, user_id) VALUES (1, 1);` 必然失败, 导致业务逻辑抛出异常, 下面的更新操作也不会发生, 

Spring Data JPA + Spring Boot 实现如下:
