...
> 为什么控制反转可以降低代码之间的耦合度?
>
> 对象不再自己去控制依赖的创建和获取，而是把这种“控制”交给外部（通常是框架或DI容器）
>
> 没有 IoC 的时候：
>
> ```java
...
> ```
>
> `UserService` 强依赖于 `UserRepository` 的具体实现，耦合很高
>
> - 如果未来要用`MockUserRepository` 或 `RemoteUserRepository`，就需要改 `UserService` 里的代码
> - 难以单元测试
>
> 有了 IoC 的时候
>
> ```java
> class UserService {
>     private final UserRepository repo; // 依赖接口/抽象
> 
>     public UserService(UserRepository repo) {
>         this.repo = repo; // IoC 容器注入实现
>     }
> }
> ```
>
> - `UserService` 只依赖 `UserRepository` 的 抽象接口，不管具体用的是什么实现
> - IoC 容器（如 Spring）根据配置决定用具体哪个 `UserRepository`
> - 解耦点：业务类只管“用”，容器负责“建”
>
> 测试困难 → 可替换依赖
>
...
> // 内部 new 了真实 UserRepository，没法替换成 Mock
> ```
>
> 有 IoC：
>
...
> UserService service = new UserService(fakeRepo); // 直接注入 Mock
> ```
>
> - 单元测试可以轻松注入替身对象（Fake/Stub/Mock）
> - 测试脱离真实数据库，速度快、可控